...
class FractEvo ():
    ...
    def __init__ (self, imagepath, imagePathMask):
        self.image = self.loadimage(imagepath)
        self.imagepath = imagepath
        self.totalPixels = self.image.size
        self.gray = cv2.cvtColor (self.image, cv2.COLOR_BGR2GRAY)
        self.maskFractevo = cv2.imread(imagePathMask)
    
    def loadimage(self,imagepath):
        
        
        image = cv2.imread(imagepath)
        image = cv2.resize(image, (516,516))
    ...
    def analyze_area_Lorenzo(self, thresMinLorenzo = 1 , thresMaxLorenzo = 256, 
                             ...
                             useMask = True):
        if useMask:
            intermediate = cv2.cvtColor(self.maskFractevo, cv2.COLOR_BGR2GRAY)
        #copyImage = self.gray.copy()
        else:
            intermediate = cv2.GaussianBlur(self.gray, (5, 5), 0)
        
        
        ret,threshImage = cv2.threshold(intermediate, thresMinLorenzo, thresMaxLorenzo, cv2.THRESH_BINARY)
...
# =============================================================================
        ...
        totalArea = intermediate.shape[0] * intermediate.shape[1]
        
        # draw bounding box
        ing2, contours, hierarchy = cv2.findContours(intermediate, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        sortedCountours = sorted(contours, key = cv2.contourArea, reverse= True)
        x,y,w,h = cv2.boundingRect(sortedCountours[0])
        cv2.rectangle(intermediate,(x,y),(x+w,y+h),(255,255,255),2)
        
        #totalAreaBoundingBox
        cv2.rectangle(threshImage,(x,y),(x+w,y+h),(255,255,255),-1)
        totalAreabounding = (threshImage > 0).sum()
        
        
        if FractEvoConstraint == False:
        
            if totalAreabounding < minArea or totalAreabounding > maxArea :
                
# =============================================================================
#                 cv2.putText(threshImage, "Area {}".format(0.1), (20, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (250, 250, 250), 1)
#                 cv2.imshow('area', threshImage)
#                 cv2.imshow('ori', intermediate)
#                 cv2.waitKey()
...
# =============================================================================
                ...
                totalArea = totalArea * 0.60
                
# =============================================================================
#                 cv2.putText(threshImage, "Area {}".format(totalAreabounding/totalArea), (20, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (250, 250, 250), 1)
#                 cv2.imshow('area', threshImage)
#                 cv2.imshow('ori', intermediate)
#                 cv2.waitKey()
#                 cv2.destroyAllWindows()
# =============================================================================
                ...
                return totalAreabounding/totalArea
...
# =============================================================================
    ...
    def analyze_blobs_Lorenzo(self, thresMinLorenzo = 100, maxBlobs = 350, previousMethod = False,
        ...
        selectedContours = [cnt for cnt in sortedContours if cv2.contourArea(cnt) > minArea and cv2.contourArea(cnt) < maxArea]
...
# =============================================================================
            ...
            # this big number will be scaled in the kmeans solution 
            r = n_blobs_Lorenzo
...
# =============================================================================
    ...
    def analyze_holes_Lorenzo (self,  thresMinLorenzo = 10, holeMinArea = 20,
        ...
        if useMask:
            grayHoles = cv2.cvtColor (self.maskFractevo, cv2.COLOR_BGR2GRAY)
        else:
            grayHoles = self.gray
            
        
        # only for display
        copyfordisplay = self.maskFractevo.copy()
        ...
        if FractEvoConstraint == False:
            if numberOfHolesLorenzo > maxNumOfHoles:
                ...
                return numberOfHolesLorenzo / maxNumOfHoles
...
#                 cv2.imshow('thresh', thresh)
#                 cv2.imshow('mask', self.maskFractevo)
#                 cv2.waitKey()
#                 cv2.destroyAllWindows()
# =============================================================================
...
# =============================================================================
    ...
    def perimeterCntVsHoles(self,thresMinLorenzo = 10, holeMinArea = 10, 
        ...
        Gray = cv2.cvtColor (self.maskFractevo, cv2.COLOR_BGR2GRAY)
        # threshold
        ...
        numberOfHoles,copyfordisplay = self._get_holes_Lorenzo ( thresMinLorenzo = thresMinLorenzo,
                                                                holeMinArea = holeMinArea,
                                                                useMask = useMask)
...
# =============================================================================
    ...
    def _get_holes_Lorenzo (self,  thresMinLorenzo = 10, 
                            holeMinArea = 10, useMask = True):
    ...
    def largeAreaReward(self, threshold = 1, targetArea = 45000):
        ...
        grayArea = cv2.cvtColor (self.maskFractevo, cv2.COLOR_BGR2GRAY)
         # threshold
...
# =============================================================================
    ...
    def jaggedlySilhoutteReward(self, goodDiffConvexTarget = 600):
        ...
        grayConv = cv2.cvtColor(self.maskFractevo, cv2.COLOR_BGR2GRAY)
        #ImageCopy = self.image.copy()
...
# working path
# =============================================================================
# path = "D:\\google drive\\A PhD Project at Godlsmiths\\MutatorML\\MLCaptureBW"
# maskpath = "D:\\google drive\\A PhD Project at Godlsmiths\\MutatorML\\MLCaptureGMask" # NEED TO SCALE UP IT IS 256 X 256
# =============================================================================

#maskpath = "D:\\aaa\\mask"
# validation
path = "D:\\google drive\\A PhD Project at Godlsmiths\\MutatorML\\MLCaptureBWvalidation"
maskpath = "D:\\google drive\\A PhD Project at Godlsmiths\\MutatorML\\MLCaptureGvalidationMask"
...
csvOutputPath = 'fractevoScores08may2018EDGEvalidation.csv'
#csvOutputPath = 'fractevoScores08may2018validationEDGE.csv'
...
#for i in range(74,84): # for testing
    print (i)
    ...
    frac = FractEvo(imagepath,imageMask)
    # 1
    areaLorenzo = frac.analyze_area_Lorenzo( thresMinLorenzo = 1 , thresMaxLorenzo = 256, 
                             maxArea = frac.maskFractevo.shape[0] * frac.maskFractevo.shape[1]*0.60,
                             minArea = frac.maskFractevo.shape[0] * frac.maskFractevo.shape[1]*0.1,
                             penalty = 4,
                             minMeanAreaLitlleCnt = 10,
                             minLenCnts = 3,
...
#                                fractionFractEVO = 0.7)
# =============================================================================
    # 2 set in a way that needs features scaling in kmeans NO UPDATED SO IT USES OLD METHOD
    nBlobs = frac.analyze_blobs_Lorenzo(thresMinLorenzo = 100, maxBlobs = 30 ,previousMethod = True,
                              fractionFractEVO = 0.5, minBlobs = 0, 
                              minArea = frac.image.shape[0]*frac.image.shape[1]/600,
                              maxArea = frac.image.shape[0]*frac.image.shape[1]/50)
    # 3
    ...
    # 9
    largeAreaReward = frac.largeAreaReward(threshold = 1,
                                           targetArea =  frac.maskFractevo.shape[0] * frac.maskFractevo.shape[1]*0.60)
    # 10
    ...
    totalScore = (areaLorenzo + nHoles + nBlobs + fractalScore + perimeterVsHolesScore
